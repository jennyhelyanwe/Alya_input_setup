...
class AlyaFormat(MeshStructure):
    ...
    def write_dom_dat(self):
        if self.geometry is not None:
            ...
            write_alya_field(filename=self.output_dir+self.name+'.nodes_xyz', field_idx=np.arange(1, self.geometry.number_of_nodes),
                             field_data=self.geometry.nodes_xyz)
            write_alya_field(filename=self.output_dir+self.name + '.tetra', field_idx=np.arange(1, self.geometry.number_of_nodes),
                             field_data=self.geometry.tetrahedrons.astype(int) + 1)
            write_alya_field(filename=self.output_dir+self.name + '.surfaces', field_idx=np.arange(1, self.geometry.number_of_triangles),
                             field_data=self.geometry.triangles.astype(int) + 1)
            write_alya_field(filename=self.output_dir+self.name + '.materials', field_idx=np.arange(1, self.materials.dict['tetra'].shape[0]),
                             field_data=self.materials.dict['tetra'])
            write_alya_field(filename=self.output_dir+self.name + '.boundaries',
                             field_idx=np.arange(1, self.boundary_element_fields.dict['mechanical-element-boundary-label'].shape[0]),
                             field_data=self.boundary_element_fields.dict['mechanical-element-boundary-label'].astype(int)+1)
            write_alya_field(filename=self.output_dir+self.name + '.sets', field_idx=np.arange(1, self.boundary_element_fields.dict['mechanical-element-boundary-label'].shape[0]),
                             field_data=self.boundary_element_fields.dict['mechanical-element-boundary-label'].astype(int)+1)
    ...
    def write_exm_dat(self):
        if self.version == 'alya-compbiomed2':
            with open(self.template_dir + self.version + '.exm.dat', 'r') as f:
                data = f.readlines()
            data = ''.join(data)
            data = data.replace('<<stimulus_field_number>>', str(self.simulation_dict['stimulus_field_name']))
