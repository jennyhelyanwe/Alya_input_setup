...
def evaluate_mesh_characteristics(csv_dir, elems, number_of_nodes):
    edges = []
    for i in range(0, len(elems)):
        edges.append([elems[i, 0], elems[i, 1]])
        edges.append([elems[i, 1], elems[i, 2]])
        edges.append([elems[i, 2], elems[i, 3]])
        edges.append([elems[i, 3], elems[i, 0]])
    edges = np.unique(np.sort(edges, axis=1), axis=0)
    unfolded_edges = np.concatenate((edges, np.flip(edges, axis=1))).astype(int)
    aux = [[] for i in range(0, nnodes, 1)]
    for i in range(0, len(unfolded_edges)):
        aux[unfolded_edges[i, 0] - 1].append(unfolded_edges[i, 1] - 1)
    neighbours = [np.array(n) for n in aux]  # Node numbers starting 0
    aux = None
    np.savetxt(csv_dir+'neighbours.csv', neighbours, delimiter=',')
...
#
def _evaluate_hybrid_rodero_fibres(fibres_nodes, number_of_nodes):
...
def _plug_fibres(self):
    ...
    for k in range(0, 4):
        ...
        plug_elems = np.nonzero(self.lvrvbase_elems == k + 7)[0]  # Plug element is labelled as 7 to 10.
        plug_nodes = np.unique(self.elems[plug_elems])
        plug_nodes_coords = self.nodes[plug_nodes - 1, :]

        non_plug_elems = self.elems[np.nonzero(self.lvrvbase_elems <= 2)[0]]
        ...
        ring_nodes_coords = self.nodes[ring_nodes - 1, :]
        ...
        mask = np.zeros(self.nnodes)
        mask[non_plug_nodes - 1] = 1
        for i in range(0, len(sorted_plug_nodes)):
            all_closest_nodes = self.neighbours[sorted_plug_nodes[i] - 1]
            ...
            if (len(closest_nodes) == 0):
                ...
                for i in range(0, len(all_closest_nodes)):
                    temp = temp + list(self.neighbours[all_closest_nodes[i]])
                all_closest_nodes = np.unique(np.array(temp))
                closest_nodes = all_closest_nodes[np.where(mask[all_closest_nodes] > 0)]
            weight = 0.4
            fibres = np.concatenate(
                (self.ortho[closest_nodes, :], weight * np.array([[1, 0, 0, 0, 1, 0, 0, 0, 1]], dtype=float)))
            new_fibres = np.mean(fibres, axis=0)
            for j in [0, 3, 6]:
                new_fibres[j:j + 3] = self._normalise_vector(new_fibres[j:j + 3])
            self.ortho[sorted_plug_nodes[i] - 1, :] = new_fibres
