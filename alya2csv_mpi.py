...
def write_geometry(number_of_blocks):
    ...
    np.savetxt(os.path.join(outputfolder, project_name + '_point_coordinates.csv'), point_coordinates,
               delimiter=',')
    np.savetxt(os.path.join(outputfolder, project_name + 'connectivity.csv'), connectivity, delimiter=',')
...
def write_variable_percell(varname, iteration, number_of_blocks):
    ...
    if 'MATER' in varname:  # this is very particular
        ...
        for elem_alya_id, elem_ensi_id in element_alya2ensi.items():
            np.savetxt(os.path.join(outputfolder, fmt % (project_name, elem_ensi_id['Name'].ljust(80), varname, iteration)), data2write,
                   delimiter=',')
    else:
...
def write_variable_pernode(varname, iteration, number_of_blocks):
    ...
    if data['header']['VariableType'] == 'scalar':
        ...
        np.savetxt(os.path.join(outputfolder, fmt % (project_name, 'scalar', varname, iteration)),data2write,
                   delimiter=',')
        ...
        np.savetxt(os.path.join(outputfolder, fmt % (project_name, 'vector', varname, iteration)), data2write,
                   delimiter=',')
    else:
